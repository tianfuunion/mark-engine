<?php
    ...
    class Request
    {
         ...
         * @return string
         */
        public function domain_tp(bool $port = false): string
        {
        ...
        {
            $host = !empty($url) ? parse_url($url, PHP_URL_HOST) : $_SERVER['HTTP_HOST'] ?? '';
            if (empty($host)) {
                return null;
            }
         ...
         * @return string
         */
        public function rootDomain($url = ""): string
        {
            ...
            if (!$this->baseFile) {
                ...
                if (!$this->isCli()) {
                    ...
                    if (basename($this->server('SCRIPT_NAME')) === $script_name) {
                        ...
                        $this->server('DOCUMENT_ROOT') && strpos(
                            $this->server('SCRIPT_FILENAME'), $this->server('DOCUMENT_ROOT')
                        ) === 0) {
        ...
        public function pathinfo(): string
        {
            if (is_null($this->pathinfo)) {
                if (isset($_GET[$this->varPathinfo])) {
                    ...
                    $pathinfo = strpos($this->server('REQUEST_URI'), '?') ? strstr($this->server('REQUEST_URI'), '?', true) : $this->server(
                        'REQUEST_URI'
                    );
                }
        ...
        public function has(string $name, string $type = 'param', bool $checkEmpty = false): bool
        {
            if (
            !in_array(
                $type, ['param', 'get', 'post', 'put', 'patch', 'route', 'delete', 'cookie', 'session', 'env', 'request', 'server', 'header',
                    'file']
            )) {
                return false;
            }
        ...
        {
            if ($this->server('HTTP_VIA') && stripos($this->server('HTTP_VIA'), "wap") !== false) {
                return true;
            }

            if ($this->server('HTTP_ACCEPT') && strpos(strtoupper($this->server('HTTP_ACCEPT')), "VND.WAP.WML")) {
                return true;
            }
            ...
            if (
                $this->server('HTTP_USER_AGENT') && preg_match(
         ...
         * @return string
         */
        public static function host($host = "", bool $strict = false): string
        {
            if (empty($host)) {
                $host = (string)($_SERVER['HTTP_X_REAL_HOST'] ?: $_SERVER['HTTP_HOST']);
            }
