...
class Curl {
    ...
    public function execute() {
        ...
        curl_setopt($this->curl, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT'] . ' Mark/1.0.0');
        ...
        // curl_setopt($this->curl, CURLOPT_REFERER, "http://XXX"); // 在HTTP请求头中"Referer: "的内容。
        ...
        if ($this->cookie) {
            ...
            curl_setopt($this->curl, CURLOPT_COOKIESESSION, $this->cookie);
        }
        ...
        if ($this->responseHeader) {
            curl_setopt($this->curl, CURLOPT_HEADER, $this->responseHeader);//设置响应头信息是否返回
        }

        // 将curl_exec()获取的信息以文件流的形式返回,而不是直接输出。(自定义默认不输出)
        if ($this->transfer) {
            curl_setopt($this->curl, CURLOPT_RETURNTRANSFER, $this->transfer);
        }

        switch ($this->method) {
            case 'get':
                curl_setopt($this->curl, CURLOPT_HTTPGET, true); //设置为get请求
                // 一个用来设置HTTP头字段的数组。使用如下的形式的数组进行设置
                ...
                break;
            case 'post':
                curl_setopt($this->curl, CURLOPT_POST, true); //设置为post请求
                // 一个用来设置HTTP头字段的数组。使用如下的形式的数组进行设置
                curl_setopt($this->curl, CURLOPT_HTTPHEADER, $this->getHeader());
            ...
            case 'upload':
                 ...
                 *
                 */
                curl_setopt($this->curl, CURLOPT_POST, 1); //设置为post请求
                // 从php5.2开始,要上传文件,必须给CURLOPT_POSTFIELDS传递数组,而不是字符串。
                // 也只有传递数组,http头部的"Content-Type"才会设置成"multipart/form-data"
                // curl_setopt($this->curl, CURLOPT_HTTPHEADER, $this->uploadheader);
                curl_setopt($this->curl, CURLOPT_HTTPHEADER, $this->getHeader());

                // curl_setopt($this->curl, CURLOPT_POSTFIELDS, http_build_query($this->formData));
                // curl_setopt($this->curl, CURLOPT_POSTFIELDS, $this->formData);
                // curl_setopt($this->curl, CURLOPT_POSTFIELDS, $this->fileData);

                // curl_setopt($this->curl, CURLOPT_POSTFIELDS, array_merge($this->fileData, http_build_query($this->formData)));
                curl_setopt($this->curl, CURLOPT_POSTFIELDS, array_merge($this->fileData, $this->formData));

                // curl_setopt($this->curl, CURLOPT_PUT, 1);
                // curl_setopt($this->curl, CURLOPT_INFILE, realpath($file));
                // curl_setopt($this->curl, CURLOPT_INFILE, $this->formData);
                // curl_setopt($this->curl, CURLOPT_INFILESIZE, $this->upfilesize);
                ...
                curl_setopt($this->curl, CURLOPT_INFILESIZE, $this->read_stream_size);
                curl_setopt($this->curl, CURLOPT_UPLOAD, true);
                } else {
                curl_setopt($this->curl, CURLOPT_POSTFIELDS, $this->formData);
                }
            ...
            case 'download':
                ...
                // curl_setopt($this->curl, CURLOPT_USERPWD, "peter.zhou:123456");
                // $this->outfile = fopen("desc.txt", "wb");
                // $this->outfile = fopen($this->filePath . "/" . $this->fileName, "wb");
                // curl_setopt($this->curl, CURLOPT_FILE, $this->outfile);
        ...
        if ($this->responseHeader) {
            ...
            // 获得响应结果里的：头大小
            $this->responseHeaderSize = $headerSize = curl_getinfo($this->curl, CURLINFO_HEADER_SIZE);
            // 根据头大小去获取头信息内容
